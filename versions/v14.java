...
public class DaliScriptMod implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            ...
            dispatcher.register(
                CommandManager.literal("daliscript")
                    .executes(ctx -> {
                        ...
                        systemOn = !systemOn;
                        send(p, systemOn ? "§aSYSTEM ON V15" : "§cSYSTEM OFF");
            ...
            // 🧠 GUI ENTRY (placeholder for real Screen UI)
            dispatcher.register(
                CommandManager.literal("daliscriptgui")
                    .executes(ctx -> {
                        ServerPlayerEntity p = ctx.getSource().getPlayer();
                        if (!isOwnerOrAdmin(p)) return 0;

                        send(p, "§bGUI OPENED (Control Panel V15)");
                        send(p, "§7Use /daliscript gamemode 0-3");
                        send(p, "§7Use /daliscript cheat");
                ...
                CommandManager.literal("daliscript")
                    .then(CommandManager.literal("cheat")
                        .executes(ctx -> {
                            ...
                            cheatMode = !cheatMode;
                            send(p, cheatMode ? "§bCHEAT MODE ON" : "§cCHEAT MODE OFF");
    ...
    private void tick(MinecraftServer server) {
        ...
        for (ServerPlayerEntity p : server.getPlayerManager().getPlayerList()) {
            ...
            // 🛡 ANTI CHEAT (unless cheatMode OWNER)
            ...
            if (w >= 5) {
                p.networkHandler.disconnect(Text.literal("BANNED BY DaliScript V15"));
    ...
    private boolean isOwnerOrAdmin(ServerPlayerEntity p) {
        return isOwner(p) || (p != null && admins.contains(p.getUuid()));
    }
    ...
    private void log(String msg) {
        System.out.println("[DaliScript V15] " + msg);
    }
}